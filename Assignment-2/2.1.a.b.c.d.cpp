// Created: 15th March 2024 07:22 AM
// By: Uttam
...
int main() {
    ...
    for (int i = 0; i < num_polygons; i++) {
        ...
        if (convex_polygon) {
            ...
            for (int j = 1; j < num_vertices - 1; j++) {
                greedy_cost += dist(convex_polygon[0], convex_polygon[j]) +
                               dist(convex_polygon[j], convex_polygon[j + 1]) +
                               dist(convex_polygon[j + 1], convex_polygon[0]);
            }
            printf("Cost for Convex Polygon %d (Greedy): %.2f\n", i + 1, greedy_cost);
    ...
    if (file) {
        fprintf(file, "Polygon,Vertices,Cost (Recursive),Cost (Dynamic Programming), Cost(Greedy Approach)\n");
        for (int i = 0; i < num_polygons; i++) {
            int num_vertices = rand() % (max_vertices - min_vertices + 1) + min_vertices;
            Point* convex_polygon = generate_convex_polygon_from_circle(num_points, num_vertices);
            if (convex_polygon) {
                ...
                fprintf(file, ",%.2f,%.2f,%.2f\n", min_cost_rec, min_cost_dp, greedy_cost);
                free(convex_polygon);
            }
        }
        fclose(file);
